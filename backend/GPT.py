...
class ChatGPT:
    ...
    @staticmethod
    def _get_gpt_res(gpt_args: dict):
        """
        ...
        """
        gpt_response = openai.ChatCompletion.create(**gpt_args)
        res_msg = gpt_response["choices"]
        token_info = gpt_response["usage"].get("total_tokens")
    ...
    def chat(self, messages: list, n: int = 1) -> str:
        ...
        gpt_response, token_info = self._get_gpt_res(gpt_args)
        res = []
        for gpt_res in gpt_response:
            msg = gpt_res["message"].get("content")
...
class ChatGPT_func(ChatGPT):
    ...
    def __init__(
        ...
        assert (model_name == "gpt-3.5-turbo-0613") or (
            model_name == "gpt-4-0613"
        ), "올바른 모델 이름을 넣으십쇼...... 개발일 기준 function_calling은 0613 모델들만 지원합니다..."

        super().__init__(api_key, model_name, concept, temp)
    ...
    def _chat_res_parser(chat_res: list, return_args_only: bool):
        ...
        gpt_func_lst = []
        for res in chat_res:
            if func_call := res["message"].get("function_call"):
                if return_args_only:
    ...
    def chat(
        ...
        res_msg, token_info = self._get_gpt_res(gpt_args)
        ...
        for parsed_item in parsed_res:
            if parsed_item["used_func"]:
                ...
                gpt_func_res, token_func_info = self._get_gpt_res(gpt_func_args)
                token_info += token_func_info

                res.append(
                    {
                        "content": gpt_func_res[0]["message"].get("content"),
                        "used_func": func_name,
...
class GPTIntentRecognizer(ChatGPT_func):
    def __init__(
        self,
        api_key: str,
        model_name: str = "gpt-3.5-turbo-0613",
        ...
        super().__init__(api_key, model_name, concept, temp, func_desc, functions)
    ...
    def chat(self, messages: list, ensure_func_name: str) -> dict:
        prompt = (
            [{"role": "system", "content": self.concept}] + messages
            if self.concept is not None
            else messages
        ...
        res_msg, _ = self._get_gpt_res(gpt_args)

        recog_res = res_msg[0]["message"].get("function_call")["arguments"]

        res_dict = ast.literal_eval(recog_res)
...
class GPTTokenizer:
    """
    `OpenAI`의 API를 이용할 때 요청 당 토큰을 핸들링하기 위한 class.\n
    `GPT-3.5-turbo`와 `GPT-4` 계열 모델을 위한 `tiktoken`의 `cl100k_base` tokenizer를 활용합니다.
    - Attribute
        - None
    """
    ...
    def get_token_len(self, content):
        """
        토큰 개수 세주는 놈이에여
        - Attribute
            - `content`(str, req): 토큰화 할 텍스트
        - Return
            - int: 입력한 텍스트의 토큰 개수
        """
    ...
    def num_tokens_from_functions(self, functions):
        """Return the number of tokens used by a list of functions."""

        num_tokens = 0
        for function in functions:
            function_tokens = len(self.encoding.encode(function["name"]))
            function_tokens += len(self.encoding.encode(function["description"]))

            if "parameters" in function:
                parameters = function["parameters"]
                if "properties" in parameters:
                    for propertiesKey in parameters["properties"]:
                        function_tokens += len(self.encoding.encode(propertiesKey))
                        v = parameters["properties"][propertiesKey]
                        for field in v:
                            if field == "type":
                                function_tokens += 2
                                function_tokens += len(self.encoding.encode(v["type"]))
                            elif field == "description":
                                function_tokens += 2
                                function_tokens += len(
                                    self.encoding.encode(v["description"])
                                )
                            elif field == "enum":
                                function_tokens -= 3
                                for o in v["enum"]:
                                    if not isinstance(o, str):
                                        o = str(o)
                                    function_tokens += 3
                                    function_tokens += len(self.encoding.encode(o))
                            else:
